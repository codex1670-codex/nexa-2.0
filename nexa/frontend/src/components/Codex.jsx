...
export default function Codex({ navigate, currentUser }) {
  ...
  const [log, setLog] = useState([{ who: 'codex', text: "Hi, I'm Codex. Try: \"follow priya\" or \"message rahul: hey!\"" }]);
    ...
    <>
      ...
      {open && (
        <div className="glass-card codex-panel">
          ...
          <div className="codex-hint">Real actions only — every reply reflects what actually happened.</div>
          <form className="codex-input-row" onSubmit={submit}>
            <input placeholder="Tell Codex what to do…" value={input} onChange={e => setInput(e.target.value)} />
          </form>
        </div>
      )}
    </>
